...
It had nowhere to put the lender. A financing provider was correctly kept out of the
...
WHAT CHANGED
  ...
  lenders             a new LC array, one item per party the source states is PROVIDING
                      financing. {name} only -- `lender_role` exists in the target model
                      with no published vocabulary, and inventing one to fill a column is
                      not preserving a distinction.
...
def test_prompt() -> None:
    ...
    check("LENDERS block delivered", "LENDERS:" in flat, True)
    check("financing_advisory is in the delivered specialty list",
          "financing_advisory —" in flat, True)
    check("lenders response slot", '"lenders": []' in flat, True)
    ...
    check("an arranger is an advisor, not a lender",
          "A firm that ARRANGES financing is a financing advisor, not a lender" in flat, True)
    # The inference this must not make.
    check("no reasoning that an arranger must also be lending",
          "do not reason that a firm arranging financing must also be lending" in flat, True)
    ...
    check("an acquirer does not lend to itself",
          "not lending to itself" in flat, True)
    ...
    check("LC version is 0.12", lc._VERSION, "0.12")
...
def test_parser() -> None:
    fn = getattr(lc, "_lenders_json", None)
    if fn is None:
        print(f"  {FAIL}  _lenders_json is missing")
        _failures.append("_lenders_json is missing")
        return

    print("\nOne lender, and many:")
    check("one lender", json.loads(fn({"lenders": [{"name": "Blackstone Credit"}]}, log, 1)),
          [{"name": "Blackstone Credit"}])
    out = json.loads(fn({"lenders": [{"name": "Blackstone Credit"},
                                     {"name": "Bank of America"}]}, log, 1))
    check("two lenders are two facts", len(out), 2)
    check("named independently", [i["name"] for i in out],
          ["Blackstone Credit", "Bank of America"])

    print("\nNo invented subtype:")
    out = json.loads(fn({"lenders": [{"name": "X", "lender_role": "AGENT",
                                      "role": "lead"}]}, log, 1))
    check("only the name is kept", out, [{"name": "X"}])

    print("\nAlways an array:")
    check("empty stays []", fn({"lenders": []}, log, 1), "[]")
    check("a missing key is still []", fn({}, log, 1), "[]")
    check("a non-list is recorded empty, not raised",
          fn({"lenders": "Bank of America"}, log, 1), "[]")
    check("a nameless entry is not a lender",
          json.loads(fn({"lenders": [{"name": " "}, {"name": "Y"}]}, log, 1)),
          [{"name": "Y"}])
...
def _ready(conn) -> bool:
    ...
    return "lenders" in cols and hasattr(ow, "_write_lender_observations")


def _seed(conn, *, lenders, advisors, eid=1, txn="tc_1"):
    ...
    conn.execute("INSERT INTO staging_extraction (extraction_id, source_raw_id, status,"
                 " transaction_cluster_id, lenders, lc_prompt_version)"
                 " VALUES (?,1,'CLUSTERED',?,?,?)",
                 (eid, txn, json.dumps(lenders), "low_confidence_extraction:0.12"))
    for a in advisors:
...
def _lender_rows(conn):
    return conn.execute("SELECT * FROM transaction_field_observation"
                        " WHERE field_name='lender_party' ORDER BY observation_id").fetchall()
...
def test_examples() -> None:
    conn = _fresh()
    if not _ready(conn):
        print(f"  {FAIL}  migration 017 / the lender writer are not present")
        _failures.append("migration 017 / the lender writer are not present")
        conn.close(); return

    print('\n"X provided committed debt financing" -> LENDER:')
    _seed(conn, lenders=[{"name": "X Capital"}], advisors=[])
    rows = _lender_rows(conn)
    check("one lender observation", len(rows), 1)
    ...
    conn = _fresh()
    _seed(conn, lenders=[], advisors=[{"name": "Y & Co", "specialty": "financing_advisory"}])
    check("one advisor", [(a["name"], a["specialty"]) for a in _advisors(conn)],
          [("Y & Co", "financing_advisory")])
    # The point of the whole slice: arranging alone never produces a lender.
    check("and NO lender", len(_lender_rows(conn)), 0)
    conn.close()

    print('\n"Z served as financial advisor and arranged the financing" -> TWO advisors:')
    conn = _fresh()
    _seed(conn, lenders=[], advisors=[
    ...
    check("and no lender appeared", len(_lender_rows(conn)), 0)
    conn.close()

    print("\nTwo lenders are two facts, with per-fact provenance:")
    conn = _fresh()
    _seed(conn, lenders=[{"name": "Blackstone Credit"}, {"name": "Bank of America"}],
          advisors=[])
    rows = _lender_rows(conn)
    ...
    check("no lender carries a subtype",
          any(set(json.loads(r["field_value"])) - {"name"} for r in rows), False)

    print("\nLenders are preserved, never reconciled:")
    check("lender_party is absent from _FIELDS", "lender_party" in dict(agg._FIELDS), False)
    fo = agg._load_observation_input(conn).get("tc_1", {}).get("field_observations", {})
    check("and never reaches the resolver", "lender_party" in fo, False)
    conn.close()
