...
**Version:** 0.13 (the terms axis becomes canonical)
...
- legacy_read_only: MERGER | REVERSE_MERGER | MINORITY_INVESTMENT | consortium | MARKET_CAPITALIZATION
...
typed; `consortium` is not a buyer classification, because classification
describes an individual firm and two firms buying together may be different kinds of
buyer; and `MARKET_CAPITALIZATION` is not a deal-value fact at all — a market cap is a
property of the company, not of the transaction, and nothing was bought at that price.
A stored market cap is never resolved into consideration. If observations disagree
between a legacy value and a current one for the same fact, prefer the current
representation.
...
| 0.10 | 2026-08-27 | **Source-stated multiples become rows.** Stage 9 writes each multiple the source itself stated into `transaction_multiple` with `source_flag = 'as_reported'`, now that clustering has produced a transaction_id to write against. `quality` and `denominator_financial_id` are NULL by rule, not by omission: the three quality values all report a calculation outcome and nothing was calculated, and "Expected on calculated rows" (V3 §6) is a scoped expectation whose scope excludes as-reported rows. **No value derivation changes.** `_compute_multiples` is untouched, the four flat `ev_to_*` columns are computed and exported exactly as before, and nothing is back-solved in either direction -- a stated multiple never yields an EBITDA, and a price and an EBITDA never yield a multiple. Re-aggregation replaces this transaction's as-reported rows and is scoped to `source_flag = 'as_reported'`, so it cannot touch a calculated row it does not own. Calculated rows are not written yet. |
| 0.11 | 2026-08-27 | **The assumed 100 is removed.** `_resolve_pct_acquired` defaulted a silent control event to 100 ('assumed'), and that number flowed into `implied_equity_value`, `implied_enterprise_value` and the calculated multiples indistinguishably from a stated fact. Unstated is now None, and None means "the source did not say". **`_derive_implied_equity` is unchanged** -- it already refused a null pct, which is precisely the ruling. **`_derive_transaction_value` now takes a control boolean instead of a percentage**: the 50 threshold was only ever a proxy for "is this a control deal", and the event type plus the minority signal answer that without a number. The predicate is the same condition that produced the assumption, so branch selection is unchanged on every row -- only the invented figure is gone. A source-stated TRANSACTION_VALUE still returns before any of this and needs no percentage. `pct_acquired_source` is never 'assumed' again. The dead `PER_SHARE_X_SHARES` basis is removed: `sec_shares` was hardcoded None so it never fired, and it multiplied by TOTAL shares outstanding while claiming a stake-level value. Consequence, recorded rather than hidden: a control deal that states no percentage now yields no `implied_equity_value`, no implied EV from that route and no calculated multiple -- those figures rested on the assumption. A source-stated multiple is unaffected, and a stated 100 (HC 0.32) keeps everything it had. |
| 0.12 | 2026-08-27 | **Source-stated revenue and EBITDA become normalized rows.** Stage 9 writes each resolved company financial to `transaction_financial` with the value reconciliation already chose -- never a staging read -- so an unresolved conflict produces no row, exactly as it does for multiples. **Currency belongs to the row**: each metric carries the currency anchored to its own amount rather than the shared `financials_currency`, which is null whenever the two metrics disagree and would strip a currency from both figures that each of them had. `_anchor_metric_qualifiers` already computed those per-metric currencies and discarded them; it now returns them. **Source-stated only** -- `is_calculated` is 0 on every row, nothing is derived to fill the table, and no figure is ever recovered from a multiple. **FX stays null**, because those fields record a conversion that was performed and none was. **Period types are carried, not translated**: ANNUAL stays ANNUAL, INTERIM_YTD stays INTERIM_YTD, and precision is read off the stored period end's own shape so a bare year is never expanded to a day. Balance-sheet metrics are deliberately not written yet. **No value derivation changes**: `_compute_multiples`, the four flat `ev_to_*` columns, `multiple_quality` and export are untouched, and the flat `target_revenue` / `target_ebitda` columns keep their existing values. Precision vocabulary is normalized onto the canonical `exact/month/quarter/year` across both normalized tables rather than leaving the two that 012 would otherwise have established. |
| 0.13 | 2026-08-27 | **The second disclosure axis becomes canonical.** `transaction_terms_disclosure_status` is reconciled and persisted exactly as `financials_disclosure_status` is, so the deal-terms axis is a canonical fact rather than one that stops at staging. Stage 9's owned column count moves 120 -> 121. No derivation reads either axis; they are collected facts carried through. |
