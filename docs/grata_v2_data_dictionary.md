...
| `target_type` | Structural type of target/object acquired. | ENUM | Source / researcher | **V3 (§T3): `STANDALONE_COMPANY` · `SUBSIDIARY` · `BUSINESS_UNIT` · `ASSETS` · unknown-null.** `SPINCO` is **not** a V3 value — it is implied by `event_type = SPIN_OFF / SPLIT_OFF`, and no replacement flag is created. One canonical column, not legacy + V2. Asset sub-classification when `target_type = ASSETS` is settled by **§T13** (`asset_type`, eleven values plus null). Per-value definitions and the transaction-form rule are in §T3. |
