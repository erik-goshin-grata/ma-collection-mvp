...
| `role` | CHANGE | Exists | The participant's role in the transaction | ENUM — TARGET · BUYER · SELLER · INVESTOR · PARENT_ACQUIRER · PARENT_SELLER · SPONSOR_BUYER · SPONSOR_SELLER · ADVISOR · FINANCING_PROVIDER · JV_PARTNER · UNDERWRITER | Collected | | `ADVISOR` replaces the two side-specific advisor roles — side comes from the advised party. **`PARENT_ACQUIRER` added**: its absence was an omission, not a decision. Sponsor side is explicit because it is meaningful role information. **`LENDER` → `FINANCING_PROVIDER` (2026-08-27)**: a party that provides, commits, or leads the provision of transaction financing. `LENDER` named an instrument and a technical capacity; the participation collected is the broader fact, and nothing about the instrument or the firm's exact capacity is classified. The distinction from `ADVISOR` + `advisor_specialty = financing_advisory` is unchanged: arranging or advising on financing does not alone establish `FINANCING_PROVIDER`, and a party may hold both participations when both are source-established. **Open, and deliberately unresolved**: a party underwriting a financing commitment may fall inside this role, which would make the separate `UNDERWRITER` value narrower than it appears. `UNDERWRITER` stays unauthored and no underwriting semantics are decided here |
