...
| 8 | Derived **flags AND valuations** not computed (equity/implied/EV/multiples ~0; and `is_take_private` / `is_divestiture` / `is_add_on` / `is_de_spac` all 0) | No derivation step in this run; share counts (SEC) not fetched; minority not grossed up. These are the same inferences done **manually in prior exercises** — not extraction misses. | **LLM captures primitives; a deterministic job computes all derived fields** — flags (from deal_type + party types + target_status) and valuations. Never the LLM. Formalizes the prior manual inference. See §5. | Code + schema + integration |
...
| 12 | **Seller sponsor field missing** — we have `acquirer_sponsor_name` but no seller-side equivalent | Asymmetric schema: buy-side sponsor captured; sell-side sponsor has nowhere to go (gets conflated into `parent_seller_name`). BMC Helix: Montagu = acquirer sponsor ✓, but KKR (seller's sponsor, "KKR-owned BMC") isn't captured as a sponsor | Add `parent_seller_sponsor_name` (+ ticker/description) for symmetry. Seller-sponsor is a real sourcing signal (which PE firms are exiting). | Schema (+ prompt to fill) |
