...
| 2 | Advisor recall ~67% | One-URL ingest under-reads multi-source deals (1-URL 24% miss vs 2+-URL 42% miss); ML also has non-URL advisor feeds | Multi-source ingest + cluster for the definitive test; accept that some ML advisors are unbeatable from URLs | Ingest / data |
